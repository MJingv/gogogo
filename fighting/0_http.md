...
https://mp.weixin.qq.com/s/oqMb-8E9wML5aV4Z6mLM0Q

历史
从文本协议进化到加密二进制流

- 1997年开始迭代，http/1.1稳定
- 2015年http/2发布，35%网页在使用
- 2022年http/3发布，27%网页在用

优先级进化

- http/1.1：无优先级概念
    - 开启keep-alive，fifo队列先入先出
    - 缺点队头阻塞
- http/2：过度设计
    - 多路复用，在一个tcp并发多个http请求。需要调度策略和优先级
    - 调度思路
        - 队列
        - round robin 并发流式
        - 并发流式+时间片不同（车道模型）
    - 优先级依赖树+权重分级（256weight）+动态修改优先级+独占式标志位
- http/3：新
    - urgency（8个优先级）+动态修改优先级+增量更新开关（interleaved）
    - 流程
        - p0～p8依次传输
        - 同级
            - 增量传，交错传输
